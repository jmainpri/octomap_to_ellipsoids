...
void GaussianMixture::initEM_3DBox(int nState, Matrix& DataSet, BBox box)
{
    this->nState = nState;
    this->dim = DataSet.ColumnSize();
    priors = new float[nState];
    ...
    double x_max = box.xmax, y_max = box.ymax, z_max = box.zmax;
    double x_min = box.xmin, y_min = box.ymin, z_min = box.zmin;

    cout << "x_max : " << x_max << " , y_max : " << y_max << " , z_max : " << z_max << endl;
    cout << "x_min : " << x_min << " , y_min : " << y_min << " , z_min : " << z_min << endl;

    int nb_cells = std::pow( double(this->nState), 1/3. );
    int offset = on_border ? 0 : 1 ;
    ...
    for( int i=offset; i<nb_cells+offset; i++ )
    {
        for( int j=offset; j<nb_cells+offset; j++ )
        {
            for( int k=offset; k<nb_cells+offset; k++ )
            {
                ...
                mean[0] = x_min + double(i)*(x_max-x_min)/double(divisions);
                mean[1] = y_min + double(j)*(y_max-y_min)/double(divisions);
                mean[2] = z_min + double(k)*(z_max-z_min)/double(divisions);

                mu.SetRow( mean, id ); /* initiate the means computed before */
                sigma[id] = Matrix( dim, dim );
                priors[id] = 1.0f / nState ; /* set equi-probables states */

                sigma[id] = unity * 0.02;
                // sigma[id] += unity * 1e-5f; /* prevents this matrix from being non-inversible */

                cout.precision(3);
                // cout << "c (" << id << ") : " << mean[0] << " , " << mean[1] << " , " << mean[2] << endl;
            }
        }
    }
}
...
{
    this->nState = nState;
    dim = DataSet.ColumnSize();

    point points = new point_t[ DataSet.RowSize() ];
    for( int i=0; i<DataSet.RowSize(); i++ )
    {
        points[i].x = Eigen::VectorXd::Zero( this->dim );

        for( int j=0; j<dim; j++ )
        {
            points[i].x[j] = DataSet( i, j );
        }
    }

    point cents = lloyd( points, DataSet.RowSize(), this->nState );
    ...
    this->nState = nb_states;
    priors = new float[this->nState];
    mu.Resize( this->nState, dim );
    sigma = new Matrix[this->nState];
    ...
    for( int i=0; i<this->nState; i++ )
    {
        int id = i; // Get centroid id
        while( cents[id].group < 0 ) id++;

        priors[id] = 1.0f /  this->nState ; /* set equi-probables states */
        ...
        mu.SetRow( mean, id );
        cout << "x[" << i << "] : " << cents[i].x.transpose() << endl;
    }
...
int GaussianMixture::doEM(Matrix DataSet, int max_iter)
{
    ...
    while(true)
    {
        ...
        std::cout << "gmm iter : " << iter << ", loglike : " << log_lik << std::endl;
        //saveParams(strtmp);
